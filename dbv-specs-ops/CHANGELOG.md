...
### Añadido
- **Soporte oficial nativo para Android (MVP de solo lectura, distribuible vía Google Play Store y APK)**:
  - **Capa Storage Access Framework (SAF)**: Nuevo plugin interno `tauri-plugin-saf` en Rust y Kotlin para gestionar permisos y acceso al sistema de archivos restringido de Android mediante `ACTION_OPEN_DOCUMENT_TREE` y `ACTION_OPEN_DOCUMENT`.
  - **Selector de archivos directo en 1 toque**: Apertura inmediata de ficheros Markdown con un solo toque sin requerir selección previa de carpetas completas.
  - **Explorador de árbol de directorios (RF-25) sobre SAF**: Exploración recursiva de documentos y subcarpetas sobre el árbol SAF concedido.
  - **Resolución de imágenes relativas y enlaces cruzados (RF-07, RF-08A)**: Carga de imágenes locales mediante conversión transparente a `data:` URI en memoria y resolución de enlaces `..` mediante `DocumentsContract.findDocumentPath`.
  - **Streaming directo desde WhatsApp, Telegram, Gmail y ContentProviders externos**: Soporte para compartir y abrir archivos directamente desde aplicaciones de mensajería y correo mediante `contentResolver.openInputStream` en memoria, sin necesidad de descarga previa ni árbol DocumentProvider.
  - **Apertura por Intent del sistema (RF-01, RF-14)**: Integración con `ACTION_VIEW` tanto en frío (arranque directo con archivo) como en caliente (reutilización de la ventana/actividad activa sin crear instancias duplicadas).
  - **Persistencia de sesión en móvil**: Restauración automática del último documento visualizado tras suspensión o recreación de la Activity por el sistema operativo.
  - **Diseño táctil y aligeramiento de la interfaz móvil (`body.is-android`)**:
    - Ocultación de controles no aplicables en pantallas móviles (breadcrumb, tiempo de lectura, insignias, botones de edición fuera de alcance).
    - Nuevo panel flotante de Ajustes (⚙️) accesible desde la barra superior con selector de tema (Claro, Oscuro, Sepia), selector de idioma (Español, English), acceso a "Acerca de" y botón explícito para salir de la aplicación.
    - Barra de herramientas optimizada con botones táctiles de 44px o más y respeto estricto de insets de sistema (barra de estado, muescas/notch y barra de navegación).
    - Panel de Tabla de Contenidos (TOC) e Índice lateral adaptados como cajón flotante táctil responsive.
    - Panel de archivos recientes adaptado a pantalla táctil fluida.
  - **Icono nativo de la aplicación**: Sustitución de los iconos de plantilla de Tauri por el isotipo y logotipo oficial de DBV Markdown Reader en todas las densidades de pantalla (`mipmap-mdpi` a `mipmap-xxxhdpi` y adaptive icon XML).
  - **Empaquetado y firma para Google Play**: Generación del paquete Android App Bundle (`app-universal-release.aab`) firmado criptográficamente con el keystore de subida de Google Play (`upload`).

### Corregido
- **Android / SAF: Persistencia de URIs `content://` en el panel de archivos recientes**: `filter_existing()` en `src-tauri/src/lib.rs` descartaba las URIs de Android al no ser rutas de archivo POSIX locales; ahora se identifican como URIs SAF persistentes y se conservan junto a las URLs remotas.
- **Android: Fallo de permisos en auto-restauración de sesiones previas**: Si el permiso de una URI de SAF temporal caducaba, la app intentaba recargarla causando un banner de error; ahora se limpia de forma silenciosa la sesión caducada sin degradar la experiencia de usuario.
- **Android: Solapamiento con la barra de estado y gestos táctiles bloqueados**: Configurado `setOnApplyWindowInsetsListener` en `MainActivity` para asegurar que el WebView respete automáticamente la barra de estado y recorte de pantalla (*display cutout*).
- **Android / Cargo: Crash en arranque por ausencia de proveedor de criptografía rustls**: Resuelto mediante inicialización temprana con `#[ctor::ctor]` antes de la creación del hilo de Webview en Android.
...
### Corregido
- **Linux: el `.deb` no asociaba los `.md` con la app (RF-01)**: tras instalar el paquete, `DBV Markdown Reader` no aparecía en el menú "Abrir con" ni en la lista de aplicaciones predeterminadas para Markdown. El `.desktop` generado (`/usr/share/applications/DBV Markdown Reader.desktop`) salía **sin línea `MimeType=`**: `bundle.fileAssociations` declaraba `ext` pero no `mimeType`, y en Linux Tauri escribe `MimeType=` únicamente a partir de ese campo — `ext` solo se usa en Windows/macOS. Añadido `"mimeType": "text/markdown"` (el tipo que `shared-mime-info` asigna realmente a `.md`). Verificado en Ubuntu 24.04 / GNOME instalando el `.deb` compilado. Cierra el riesgo aceptado en el Adversarial Review de la Fase 24 ("el `.deb` registra la asociación … pero esto no se ha podido verificar en una máquina Linux real", `memory.md`): el supuesto era incorrecto. Contribución de Jacinto Parga ([PR #9](https://github.com/davidbuenov/dbv-md-reader/pull/9)).
- **macOS: bucle de scroll en Modo Edición que no paraba solo**: con trackpad, el scroll sincronizado entre editor y preview podía entrar en un bucle de retroalimentación imparable. `withScrollLock()` limpiaba su flag anti-eco con un único `requestAnimationFrame`, pero WebKit despacha el evento `scroll` de forma asíncrona y sin orden garantizado respecto al rAF: si el eco llegaba tarde, el listener del otro panel lo trataba como scroll real del usuario y volvía a sincronizar, y como `interpolateScroll()` no es exactamente su propia inversa, cada vuelta arrastraba unos píxeles de más. Corregido limpiando el flag cuando llega el propio eco del elemento modificado (listener `once`), con un rAF doble como red de seguridad. Contribución de Victor Estival ([PR #10](https://github.com/davidbuenov/dbv-md-reader/pull/10)).
