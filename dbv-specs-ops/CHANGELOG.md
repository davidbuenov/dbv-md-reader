...
- **Android: árbol completo de carpetas, imágenes relativas y enlaces cruzados (RF-25/RF-07/RF-08A sobre SAF), Slice 2 de `implementation_plan.md`.** Nuevos comandos en `tauri-plugin-saf` (`list_children`, `read_document`, `resolve_relative`, `read_image_data_uri`) que el Explorador de árbol, la resolución de imágenes y los enlaces relativos ya existentes reutilizan sin cambios de lógica — el frontend elige entre el comando de escritorio o su equivalente SAF solo mirando si la ruta empieza por `content://` (ver `isSafUri()` en `app.js`). Las rutas relativas con `..` suben a la carpeta padre real vía `DocumentsContract.findDocumentPath` (API 26+, con degradación silenciosa si el proveedor no lo soporta); las imágenes se incrustan como `data:` URI porque el WebView de Android no puede cargar `content://` directamente en `<img src>`. Verificado navegando una carpeta de prueba con subcarpetas, una imagen relativa (incluida una referencia `../`) y un enlace cruzado entre documentos, sin errores en `adb logcat`.
- **Android: el botón "Editar" queda deshabilitado (con el badge "Solo lectura") sobre cualquier documento abierto vía SAF**, mismo criterio ya usado para documentos remotos (`http(s)://`) — evita entrar en un Modo Edición sin comando de guardado real detrás (RF-20 sigue fuera de alcance en Android, ADR-031).
- **Android: apertura por Intent contra apps externas reales (RF-01/RF-14 sobre SAF), Slice 3 de `implementation_plan.md`.** "Abrir con"/compartir un `.md` desde otra app (gestor de archivos, y cualquier proveedor que declare `text/markdown` como MIME real) ahora abre el documento tanto en frío (app cerrada) como con la app ya en marcha — en caliente sustituye el documento actual en la única ventana en vez de crear una nueva (excepción de RF-14 en Android, sin equivalente a "ventana nueva"). Sin código Kotlin nuevo: Tauri ya traduce el intent `ACTION_VIEW`/`onNewIntent` de Android como `RunEvent::Opened` (igual que "Open With" en macOS) — solo hacía falta una rama en `lib.rs` para `target_os = "android"`. **R1 (gap de MIME de Gmail/Drive) investigado a fondo y verificado con dispatch real de Intents (`adb shell am start`, no solo lectura del manifiesto): la regla de reserva por `pathPattern`/`pathSuffix` que preveía el plan resultó insegura en la práctica** — Android no soporta declarativamente "cualquier MIME pero solo esta extensión" en un `<intent-filter>`, y la única forma de conseguir que combinase MIME comodín con la extensión hacía que la app apareciera como candidata para *cualquier* archivo compartido (confirmado abriendo un `.pdf` sin relación con Markdown). Revertida por seguridad; R1 queda como riesgo aceptado sin mitigación adicional, tal y como preveía el plan original si la mitigación no resultaba viable.
