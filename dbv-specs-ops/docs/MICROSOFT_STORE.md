...
> **Estado:** ✅ Publicado — ficha en vivo en [apps.microsoft.com/detail/9n7bmdzgcp0s](https://apps.microsoft.com/detail/9n7bmdzgcp0s) (`9N7BMDZGCP0S`). **Resubmission `v0.16.0` pendiente de subida manual (última versión en vivo en la Store: `v0.13.0`, confirmado por el usuario — corrige el dato de `v0.8.0` que arrastraba esta nota desde antes).**
> **Última revisión:** 2026-09-19

**Nota (2026-09-19) — cambio de política, ver ADR-047 en `memory.md`:** este canal MSIX pasa a ser el **único** canal de distribución de Windows desde `v0.16.0` — se descontinúa el instalador NSIS de GitHub Releases (contraseña de la clave de firma `minisign` perdida; en vez de generar una clave nueva e invalidar el auto-update de las instalaciones NSIS existentes, se prioriza este canal, que ya funcionaba bien). `.msixbundle` de `v0.16.0` generado (`npx @choochmeque/tauri-windows-bundle build --runner npm` sobre un build **sin firmar** — sin necesidad de clave propia, la Store re-firma el paquete con la suya): `src-tauri/target/msix/dbv-md-reader_0.16.0.0.msixbundle`. Assets verificados sin placeholders (mismo checklist de siempre, los 4 iconos con cientos de colores únicos). `descripcionStore_{es,en}.md` actualizados con la delta real desde `v0.13.0` (no desde `v0.8.0`, como se asumió en un primer borrador y hubo que corregir): alertas GFM + indicador de cambios sin guardar (ya en v0.13.1, quedan como contexto) y, nuevo de verdad en `v0.16.0`, exportar a Typst + fix de colores de impresión/PDF + numeración de línea alineada. **Pendiente del usuario, fuera del alcance de Claude Code:** subir el `.msixbundle` a Partner Center junto con los textos ya actualizados y enviarlo a certificación (checklist §5).

**Nota (2026-08-20):** `descripcionStore_es.md`/`descripcionStore_en.md` actualizados para cubrir todo lo añadido entre `v0.8.0` (última versión realmente subida a Partner Center) y `v0.12.0` — el cambio de fondo es que la app pasa de "lector" a "lector y editor" (Modo Edición RF-20 con vista dividida en vivo, RF-21 gestión de conflictos, RF-22 ayuda de sintaxis, RF-23 barra de formato, RF-24 Tab/Shift+Tab, más GFM de task lists/footnotes y Always on Top de `v0.9.0`). Descripción larga, "Novedades de esta versión", lista de características (reescrita completa para evitar el hueco sin registrar que había en el punto #16) y descripción corta actualizadas en ambos idiomas; palabras clave cambian "visor md"/"md viewer" por "editor markdown"/"markdown editor" para reflejar la nueva capacidad de edición. **`.msixbundle` de `v0.12.0` ya generado (2026-08-20):** `npx @choochmeque/tauri-windows-bundle build --runner npm` sobre el build de release ya compilado (`src-tauri/target/release/dbv-md-reader.exe`, del mismo día que el tag `v0.12.0`) → `src-tauri/target/msix/dbv-md-reader_0.12.0.0.msixbundle`. Assets verificados sin placeholders (`Square150x150Logo.png`/`Square44x44Logo.png`/`StoreLogo.png`/`Wide310x150Logo.png`, todos con cientos de colores únicos — ver checklist §5.8bis y el incidente §4bis). **Pendiente del usuario, fuera del alcance de esta sesión:** (1) capturas de pantalla nuevas mostrando el Modo Edición (las actuales en `docs/assets/store/store_{es,en}_{1,2}.png` son de antes del Modo Edición); (2) subir el `.msixbundle` y los textos actualizados de `descripcionStore_{es,en}.md` a Partner Center y enviarlo a certificación (checklist §5).
...
**`displayName` fijado explícitamente a `"dbv-md-reader"` en `bundle.config.json`** (no al valor de marca "DBV Markdown Reader"): la herramienta deriva el nombre de archivo del `.exe` esperado dentro del paquete a partir de ese campo (quitando espacios), y el binario compilado real sigue siendo `dbv-md-reader.exe` (nombre de Cargo, sin cambios). Al principio se dejó sin fijar (heredaba `productName` de `tauri.conf.json`), lo cual funcionaba mientras `productName` era `"dbv-md-reader"` — pero **el rebrand a "DBV Markdown Reader" (ver más abajo) rompió ese valor por defecto** (`executableName()` pasó a esperar `DBVMarkdownReader.exe`, que no existe) y el build falló con `Executable not found` hasta fijar `displayName` de vuelta al nombre real del binario. **Efecto secundario conocido y aceptado:** el tile/nombre que Windows muestra para la app instalada desde la Store (`<Properties><DisplayName>` del manifiesto) queda como "dbv-md-reader", no "DBV Markdown Reader" como en el canal NSIS/UI — inconsistencia menor entre canales, pendiente de decidir si merece la pena resolverla con `mainBinaryName` en `tauri.conf.json` (renombraría el `.exe` compilado en ambos canales, cambio más profundo no aplicado en esta sesión).
