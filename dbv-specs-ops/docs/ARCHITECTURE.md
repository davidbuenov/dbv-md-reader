...
- **ADR-044 (2026-09-19):** Ciclo v0.16.0 — cuatro decisiones de alcance confirmadas con el usuario antes de `/plan`: (1) Homebrew se distribuye vía tap propio (`Cask` en repo nuevo `homebrew-dbv-md-reader`), no vía `homebrew-core` (no se cumplen sus criterios de notoriedad), con actualización automática desde `release-macos.yml` en vez de manual; (2) el conversor Markdown → Typst se especifica en ambos proyectos del usuario (exportar en `dbv-md-reader`, importar en `dbv-typst-editor`) pero como tareas independientes, este ciclo solo cubre la exportación; (3) el fix de pérdida de color al imprimir/exportar PDF (RF-10) entra sí o sí en este ciclo; (4) el "Exportar PDF" dedicado sin diálogo nativo (RF-28) se investiga en este mismo ciclo pero puede posponerse a v0.17.0 según el resultado del spike técnico. Ver `SPECIFICATIONS.md` RF-27/RF-28 y sección 8.
...
### macOS — Homebrew (planificado 2026-09-19, ciclo v0.16.0, ver ADR-044 en `memory.md`)

Canal de instalación adicional sobre el `.dmg` universal ya publicado por `release-macos.yml`, sin sustituir la falta de firma/notarización (ver más arriba):

- **Repo nuevo `davidbuenov/homebrew-dbv-md-reader`** (convención de Homebrew: el nombre del tap debe empezar por `homebrew-`), creado 2026-09-19. Contiene únicamente el fichero `Casks/dbv-md-reader.rb` y un `README.md` — sin código fuente, sin build propio.
- **`Cask`, no `Formula`:** es una app GUI empaquetada en `.dmg`, no un binario de línea de comandos — `Formula` es para lo segundo.
- **Actualización automática (no manual), implementada en `/build` como workflow separado, no como paso añadido a `release-macos.yml` (desviación consciente respecto al plan inicial):** `.github/workflows/update-homebrew-tap.yml`, disparado por el evento nativo `release: published` en vez de encadenarse al final de `release-macos.yml`. Motivo del cambio: `release-macos.yml` normalmente deja la Release en estado **borrador** (el maintainer sube a mano los 3 ficheros de Windows antes de publicar) — los assets de un borrador no tienen URL de descarga pública, así que calcular el sha256 en ese momento apuntaría el Cask a un fichero todavía no descargable por nadie. `release: published` cubre tanto la publicación manual desde la UI de GitHub como `workflow_dispatch` con `draft: false`, que es el momento real en que el `.dmg` queda público. El nuevo workflow usa `gh release download --pattern "*universal.dmg"` (token por defecto del repo, de solo lectura, suficiente) para obtener el asset ya público, calcula su sha256, y hace `checkout`+commit+push al repo del tap con un token de escritura dedicado (`secrets.HOMEBREW_TAP_TOKEN`, con permiso de solo "Contents: Read and write" sobre ese repo únicamente). Si la Release no trae el `.dmg` universal, el job se salta sin fallar.
